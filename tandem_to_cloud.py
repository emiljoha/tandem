...
def wf_variance_from_energy_and_spin(num_orbitals, num_particles, one_basis_spins,
                                     one_basis_energies, temperature):
    zero_spin_bool_list = pt.is_spin_zero(num_orbitals, num_particles, one_basis_spins)
    zero_spin_bool_list = np.array(zero_spin_bool_list) # We want to do numpy broadcasting magic
    no_interaction_energies = pt.basis_energies(num_orbitals, num_particles, one_basis_energies)
    # Note the numpy broadcasting. 
    energy_diff_from_ground = no_interaction_energies[0] - np.array(no_interaction_energies)
    return np.exp(energy_diff_from_ground / temperature) * zero_spin_bool_list

def sample_around_HF_wf(temperature, num_examples, wave_function, num_orbitals, num_particles,
                        one_basis_spins, one_basis_energies):
    wf_variance = wf_variance_from_energy_and_spin(num_orbitals, num_particles, one_basis_spins,
                                                   one_basis_energies, temperature)
...
def HF_generate_and_save_to_cloud_from_wf(args):
    ...
    if already_exists(cloud_path, bucket_name='tandem_10'):
        # print '%s already exists' % name
        return None
    wf_around_hf = sample_around_HF_wf(temperature, num_examples, wave_function, num_orbitals, num_particles,
                                       one_basis_spins, one_basis_energies)
